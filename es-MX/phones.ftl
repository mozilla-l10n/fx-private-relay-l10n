...
phone-onboarding-step3-body =
    { $remaining_minutes ->
        [0]
            { $remaining_seconds ->
                [1] Ingresa el código de verificación enviado al <span>{ $phone_number }</span> en un plazo de <strong>{ $remaining_seconds } segundo</strong>.
               *[other] Ingresa el código de verificación enviado al <span>{ $phone_number }</span> en un plazo de <strong>{ $remaining_seconds } segundos</strong>.
            }
       *[other]
            { $remaining_seconds ->
                [1] Ingresa el código de verificación enviado al <span>{ $phone_number }</span> en un plazo de <strong>{ $remaining_minutes }:{ NUMBER($remaining_seconds, minimumIntegerDigits: 2) } minutos</strong>.
               *[other] Ingresa el código de verificación enviado al <span>{ $phone_number }</span> en un plazo de <strong>{ $remaining_minutes }:{ NUMBER($remaining_seconds, minimumIntegerDigits: 2) } minutos</strong>.
            }
    }
