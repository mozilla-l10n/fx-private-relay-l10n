...
# Variables
#   $mask_limit (number) - the number of free email masks available
first-time-user-email-hero-primary-text-3 = En tant qu’utilisateur·rice de { -brand-name-firefox }, vous disposez de { $mask_limit } alias de messagerie gratuits. Utilisez-les pour masquer votre véritable adresse e-mail, protéger votre identité et transférer uniquement les e-mails que vous souhaitez vers votre boîte de réception.
...
first-time-user-email-how-item-1-subhead-2-html = Créez des alias directement depuis { -brand-name-firefox }, avec le <a href="{ $url }" { $attrs }>module complémentaire de { -brand-name-relay }</a>, ou depuis votre tableau de bord.
...
#   $attrs (string) - link attributes
first-time-user-email-how-item-3-subhead-2-html = <a href="{ $url }" { $attrs }>Connectez-vous</a> pour créer, étiqueter ou supprimer des alias à tout moment.
...
#   $attrs (string) - link attributes
first-time-user-email-questions-subhead-2-html = Consultez notre <a href="{ $url }" { $attrs }>centre d’assistance</a> pour obtenir de l’aide
...
api-error-account-is-inactive = Votre compte n’est pas actif.
