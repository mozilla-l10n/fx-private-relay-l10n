...
tooltip-email-domain-explanation-part-one-free = С помощью { -brand-name-premium } вы можете создавать неограниченное количество пользовательских псевдонимов { -brand-name-relay } на ходу, используя уникальный домен электронной почты { -brand-name-relay } — вам даже не придётся перед этим их создавать.
# Variables:
#   $mozmail (string): domain used by Relay masks (mozmail.com)
# "yourdomain" is used as an example custom domain. Keep it lowercase without space and in latin characters, to resemble and actual email address. Example: shopping@johndoe.mozmail.com
